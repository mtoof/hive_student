...
/*   Updated: 2022/12/14 16:24:24 by mtoof            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_check(va_list args, const char *tmp)
{
	...
	if (*tmp == 'c')
		...
		counter += ft_putnbr(va_arg(args, int), 10, 0);
	else if (*tmp == 'u')
		counter += ft_putnbr(va_arg(args, unsigned int), 10, 1);
	else if (*tmp == 'p')
	{
		counter += ft_putstr("0x");
		counter += ft_convert((size_t)va_arg(args, void *), 16, 87);
	}
	else if (*tmp == '%')
		counter += ft_putchar('%');
	return (counter);
